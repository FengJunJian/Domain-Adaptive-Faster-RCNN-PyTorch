...
at::Tensor SigmoidFocalLoss_forward_cuda(
  ...
  dim3 grid(std::min((long)(losses_size+511)/512L, 4096L));//dim3 grid(std::min(THCCeilDiv(losses_size, 512L), 4096L));
  dim3 block(512);
...
at::Tensor SigmoidFocalLoss_backward_cuda(
  ...
  dim3 grid(std::min((long)(d_logits_size+511)/512L, 4096L));//dim3 grid(std::min(THCCeilDiv(d_logits_size, 512L), 4096L));
  dim3 block(512);
